...
ChannelStateService::ChannelStateService(AsyncWebServer* server,
  ...
  _channelMqttSettingsService->addUpdateHandler(
...
void ChannelStateService::registerPinConfig(uint8_t controlPin, uint8_t homeAssistantTopicType) {
  ...
  _channelMqttSettingsService->read([&](ChannelMqttSettings& settings) {
    String mqttPath =
        utils.getMqttUniqueIdOrPath(controlPin, homeAssistantTopicType, false, settings.homeAssistantEntity);
...
void ChannelStateService::onChangeBrightness(uint32_t duty = 1) {
  uint8_t valueMax = 255;
  // calculate duty, 4095 from 2 ^ 12 - 1
  if(duty != 0){
    duty = (4095 / valueMax) * min(_state.channel.brightness, valueMax);
  } 
  // write duty to LEDC
...
void ChannelStateService::onSetupBrightness() {
  if (_state.channel.homeAssistantTopicType == HOMEASSISTANT_TOPIC_TYPE_LED) {
...
void ChannelStateService::updateStateTime() {
  update([&](ChannelState& channelState) { return StateUpdateResult::CHANGED; }, _state.channel.name);
}
