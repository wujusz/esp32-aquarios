...
#define NUM_END(off, mult) ((timeStringEnd[(off)] - '0') * (mult))
  ...
  TotalScheduledTime timeToStartSeconds(time_t currentTime,
                                        int startTime,
                                        int endTime,
    ...
    if (startTime < endTime) {                                         // start 7:00 end 23:00
      if (endDateTime > currentTime) {                                 // we have not reached endTime
        ...
        totalTime.timeToStartSeconds =
            startDateTime + TWENTY_FOUR_HOUR_DURATION - currentTime;  // we are starting next day
      }
  ...
  ScheduledTime getScheduleTimes(int startTime,
                                 int endTime,
                                 int hotTimeHour,
                                 bool enableTimeSpan,
                                 bool isHotScheduleActive,
    ...
    if (startTime > endTime) {
      schedule.scheduleEndDateTime = schedule.scheduleEndDateTime + TWENTY_FOUR_HOUR_DURATION;
    }

    TotalScheduledTime totalTime = timeToStartSeconds(
        schedule.currentTime, startTime, endTime, schedule.scheduleStartDateTime, schedule.scheduleEndDateTime);
    ...
    if (!schedule.isHotSchedule) {
      ...
      if (schedule.startTime < schedule.endTime) {
        ...
        if (!schedule.isHotScheduleAdjust) {
          ...
          schedule.isRunTaskNow =
              schedule.currentTime > (schedule.scheduleHotTimeEndDateTime - TWENTY_FOUR_HOUR_DURATION) &&
  ...
  static String getMqttUniqueIdOrPath(uint8_t controlPin,
                                      uint8_t homeAssistantTopicType,
                                      bool isUniqueIdOrPath,
    ...
    return isUniqueIdOrPath ? SettingValue::format(topicType + "-pin-" + String(controlPin) + "-#{unique_id}")
                            : SettingValue::format(topicHeader + homeAssistantEntity + "-pin-" + String(controlPin) +
                                                   "/#{unique_id}");
  }
